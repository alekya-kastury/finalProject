...
def run_query_plot(query):
    df=pd.read_sql_query(query,engine)
    c = alt.Chart(df,title='Yearly customer count of a month').mark_point().encode(x=alt.X('year:O', axis=alt.Axis(title='Year', labels=False), scale=alt.Scale(domain=list(df['year']))), y='count_of_customers')
