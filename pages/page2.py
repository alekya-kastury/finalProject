...
def run_query_plot(query):
    df=pd.read_sql_query(query,engine)
    c = alt.Chart(df,title='Yearly customer count of a month').mark_point().encode(x=alt.X('year','category:values', scale=alt.Scale(domain=[1998,2003])), y='count_of_customers')
